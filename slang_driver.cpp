...
#include "target.inc"
  ...
  FileCount = Argc;
  InputFileNames = new char*[FileCount];
  OutputFileNames = new char*[FileCount];
  int count;
  for (count = 0; count < FileCount; count++) {
    InputFileNames[count] = Argv[optind + count];
    OutputFileNames[count] = new char[ strlen(InputFileNames[count]) + 1 ];
    strcpy(OutputFileNames[count], InputFileNames[count]);
    char *idx = OutputFileNames[count] + strlen(InputFileNames[count]) - 3;
    if (strcmp(idx, ".rs")) {
    ...
    switch (OutputFileType) {
      ...
      case SlangCompilerOutput_Nothing:
        strcpy(OutputFileNames[count], "/dev/null");
        break;
...
int main(int argc, char** argv) {
  ...
  if(ParseOption(argc, argv)) {
    ...
    for (count = 0; count < FileCount; count++) {
      ...
      SLANG_CALL_AND_CHECK( slangSetSourceFromFile(slang, InputFileNames[count]) );
      ...
      SLANG_CALL_AND_CHECK( slangSetOutputToFile(slang, OutputFileNames[count]) );
